  ...
  const functionality = function (server, options) {
    return {
      crud: function (config) {
        ...
        this.route({
          method: 'GET',
          path: config.basePath+ '/_count',
          handler: function (request, reply) {
			let ret = config.bookshelfModel.where(findAllQueryParams(request, config))
			if (options.allowFilters) {
				ret = filterQueryParams(ret, request)
			}
            return ret.count().then(function (num) {
              reply(num)
            })
          },
		  validate: false,
		  isAllowed: options.allowCount,
          baseQuery: config.baseQuery,
          bookshelfModel: config.bookshelfModel,
        })
        ...
        this.route({
          ...
          path: config.basePath + '/{id}',
          handler: function (request, reply) {
            return config.bookshelfModel.where(findByIdQueryParams(request, config)).destroy().then(function () {
              reply().code(204)
            })
          },
		  validate: false,
        ...
        this.route({
          ...
          handler: function (request, reply) {
			...
			let ret = config.bookshelfModel.where(findAllQueryParams(request, config)).where('id', '!=', '0')
			if (options.allowFilters) {
				ret = filterQueryParams(ret, request)
			}
			return ret.destroy().then(function () {
              reply().code(204)
            })
          },
		  validate: false,
